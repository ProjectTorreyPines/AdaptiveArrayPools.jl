...
# `_am_peak_n_active` (one hot-path `max` in `_cuda_claim_slot!`, gated by AUTO_MANAGE).
...
@testset "CUDA auto_trim" begin
    ...
    # ── Data layer ───────────────────────────────────────────────────────────
    @testset "CuTypedPool._am_peak_n_active (default 0) + pool._trim_requested (default false)" begin
    ...
    @testset "_cuda_claim_slot! records the peak n_active in _am_peak_n_active" begin
        pool = CuAdaptiveArrayPool{0}()
        checkpoint!(pool)
