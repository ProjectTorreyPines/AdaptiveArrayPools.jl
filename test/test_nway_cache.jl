...
@testset "N-way Cache for acquire!" begin
    ...
    @testset "Type checks" begin
        ...
        @with_pool pool begin
            ...
            raw = acquire!(pool, Float64, 10, 10)
...
@testset "N-way Zero-Allocation" begin
    ...
    @testset "N-D acquire!: 4-way alternating is zero-alloc (Array)" begin
        pool = AdaptiveArrayPool()
...
@testset "Vector-based arr_wrappers cache" begin
    ...
    @testset "arr_wrappers grows correctly for multiple dimensionalities" begin
        ...
        # N=2: 2D acquire — arr_wrappers should grow to index 2
        m1 = acquire!(pool, Float64, 3, 4)
        @test length(pool.float64.arr_wrappers) >= 2
        @test pool.float64.arr_wrappers[2] !== nothing  # has a Vector{Any} for N=2

        # N=3: 3D acquire — arr_wrappers should grow to index 3
        t1 = acquire!(pool, Float64, 2, 3, 4)
    ...
    @testset "multiple element types have independent arr_wrappers" begin
        ...
        mf = acquire!(pool, Float64, 3, 4)
        mi = acquire!(pool, Int64, 5, 6)
