# Metal rand! / randn! tests
# ==========================
# The live Metal pool inherits the core _rand_impl!/_randn_impl! (which call
# Random.rand!/randn! on the acquired MtlArray — Metal.jl runs these on-device).
...
@testset "Metal rand!/randn!" begin
    ...
    @testset "rand! tuple form" begin
        ...
        @test m32 isa MtlArray{Float32, 2}
        @test size(m32) == (3, 4)
    end

    @testset "randn! default + explicit" begin
        g = randn!(pool, 100)
        @test g isa MtlArray{Float32}
        @test all(isfinite, Array(g))

        g32 = randn!(pool, Float32, 4, 5)
        @test g32 isa MtlArray{Float32, 2}
        @test size(g32) == (4, 5)
    ...
    @testset "returns MtlArray (not a view)" begin
    ...
    @testset "DisabledPool{:metal} fallbacks (GPU-native)" begin
        DM = ext.DISABLED_METAL

        v = rand!(DM, Float32, 16)
        @test v isa MtlArray{Float32}
        @test all(x -> 0.0f0 <= x < 1.0f0, Array(v))

        v2 = rand!(DM, 5, 5)                 # default Float32
        @test v2 isa MtlArray{Float32, 2}
        @test size(v2) == (5, 5)

        vt = rand!(DM, Float32, (3, 4))      # NTuple form
        @test vt isa MtlArray{Float32, 2}
        @test size(vt) == (3, 4)

        g = randn!(DM, Float32, 16)
        @test g isa MtlArray{Float32}
        @test all(isfinite, Array(g))

        gt = randn!(DM, (2, 3))              # default Float32 NTuple
        @test size(gt) == (2, 3)

        # collection rejected on the disabled pool too
        @test_throws ArgumentError rand!(DM, 1:6, 10)
    end
end
