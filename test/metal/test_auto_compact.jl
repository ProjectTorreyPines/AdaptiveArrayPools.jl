...
# =========================
# Parity with the CPU suite (test/test_auto_compact.jl) for the Metal backend.
#
...
@testset "Metal auto_compact" begin
    ...
    @testset "_run_auto_compact! resets the flag and compacts inactive Metal bloat" begin
    ...
    @testset "_maybe_auto_compact!(::MetalAdaptiveArrayPool) fires only at depth 1" begin
        pool = _metal_inactive_bloated(1_000_000, 100)
    ...
    @testset "_maybe_auto_compact! is a no-op when the Metal flag is clear" begin
        pool = _metal_inactive_bloated(1_000_000, 100)
        @test (@atomic pool._compact_requested) == false
    ...
    @testset "end-to-end: bloat auto-compacts at a @with_pool :metal boundary" begin
        ...
        # to _maybe_auto_compact!(::MetalAdaptiveArrayPool).
        @atomic pool._compact_requested = true
        ...
        @test _mcap(pool.float32.vectors[1]) < cap0         # auto-compacted, no manual compact!
        empty!(pool)
        _clear_registry!()
    end
...
AAP.disable_auto_compact!()   # leave the timer stopped for later Metal test files
