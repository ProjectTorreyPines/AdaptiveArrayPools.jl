# Metal Auto-manage END-TO-END integration + real-time tests.
# =========================================================
# Parity with the CPU suite (test/test_auto_integration.jl) for the Metal backend.
#
...
#       auto-trim every K-th tick, on a Metal pool.
#
...
@testset "Metal auto_manage integration + real-time" begin
    ...
    # GPU kernel-launch bound (~0.5 s for 300 steps); spans enough wall-time for many
    # 0.01 s ticks. (GPU `sum` over integer fills is bit-reproducible — verified.)
    N_STEPS = 300
    ...
    function run_sim_metal!(n_steps)
    ...
    @testset "A. real Timer firing mid-computation produces bit-identical results" begin
        ...
        empty!(pool)
        ref = run_sim_metal!(N_STEPS)
        slots_off = length(pool.float32.vectors)
        big_off = maximum(_mcap, pool.float32.vectors)
        @test slots_off >= 5
        ...
        @test maximum(_mcap, pool.float32.vectors) < big_off      # ★ auto-compact reclaimed the bloat

        empty!(pool); _clear_registry!()
    end

    # ── B. Real-time cadence over real elapsed seconds (Metal pool) ───────────
