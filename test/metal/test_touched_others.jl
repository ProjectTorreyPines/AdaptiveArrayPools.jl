# Metal port of test/test_touched_others.jl — depth-tagged touched-others stack
...
# dedicated "Float16 bit-7" testsets below).
...
# Metal.jl's hardware backend rejects Int128/UInt128 ("Metal does not support
# Int128 values"), so the 8-type pollution-regression testset below substitutes
# two distinct isbits struct types for those two. Named distinctly from the CPU
# test suite's TOFooA/B/C (both files load into the same top-level module during
# a full-suite run) to avoid a struct-redefinition clash.
struct MetalFallbackStructA
    x::Float32
end
struct MetalFallbackStructB
    x::Float32
end
...
@testset "touched-others: no eager checkpoint on lazy entry (pollution regression)" begin
    ...
    # (Int128/UInt128 substituted with distinct isbits structs — Metal.jl's hardware
    # backend rejects 128-bit integer element types.)
    fallback_types = (UInt16, UInt8, Int8, UInt32, Int16, UInt64, MetalFallbackStructA, MetalFallbackStructB)
...
@testset "touched-others: @with_pool integration + exception-leak recovery" begin
    # The task-local Metal pool is a process-wide-per-device singleton shared with
    ...
    f_leaf(n) = @with_pool :metal p begin
        q = acquire!(p, UInt16, n)
        length(q)
    end
    @test f_leaf(8) == 8
    tl = get_task_local_metal_pool()
...
@testset "touched-others: R=1 exception-leak recovery (lockstep across throw)" begin
    # The "@with_pool integration + exception-leak recovery" testset above only
    # exercises the two-stack (states/depths) lockstep at the default R=0. Construct
    # an R=1 pool directly (same pattern as the R=1 lockstep testset above) to verify
...
@testset "touched-others: R=1 nested-depth lockstep (fallback touches at depths 2 and 3)" begin
    # Mirrors "touched-others: depth tags are exact and monotone" above, but at R=1,
...
# Routing MUST key off `_metal_is_fallback_type(T) = !(T <: _METAL_FIXED_TYPES)`,
