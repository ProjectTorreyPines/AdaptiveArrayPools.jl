...
On Julia 1.12+, the `setfield!`-based wrapper reuse path is available, enabling zero-allocation `Array` returns — the same guarantee views had. (Julia 1.11 made `Array` a mutable struct, but its `arraylen` bug prevents safe `setfield!` reuse.) With Array now zero-alloc too, it's the natural default.
...
| Zero-alloc reuse (1.12+) | Yes (`setfield!`) | Yes (always was) |
...
### Julia ≤1.11 (Legacy Path)

v0.3 still supports Julia ≤1.11, but the zero-allocation `Array` reuse via `setfield!` only works on Julia 1.12+. On ≤1.11, `acquire!` returns a freshly constructed `Array` wrapper (the underlying buffer is still reused — only the wrapper allocates ~40 bytes). Julia 1.11 made `Array` a mutable struct, but its `arraylen` builtin uses `Memory.length` instead of `prod(size)`, which breaks the `setfield!(:ref)` path.
...
Both are zero-allocation on Julia 1.12+. The only difference is the returned wrapper type.
